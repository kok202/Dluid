...
import org.kokzoz.dluid.ai.network.layer.LayerBindManager;
...
public class ModelConverter {
    ...
    public List<Model> convert(LayerGraphManagerAnalyzer layerGraphManagerAnalyzer){
        return layerGraphManagerAnalyzer.getInputLayerIds()
                .stream()
                .map(inputLayerId -> {
                    List<org.kokzoz.dluid.domain.entity.Layer> dluidLayers = new ArrayList<>(layerGraphManagerAnalyzer.getMultiLayerManager().get(inputLayerId).values());
                    ...
                    for (org.kokzoz.dluid.domain.entity.Layer dluidLayer : dluidLayers) {
    ...
    private void recycleInputPreProcessor(ComputationGraphConfiguration.GraphBuilder graphBuilder, List<org.kokzoz.dluid.domain.entity.Layer> dluidLayers){
        for (org.kokzoz.dluid.domain.entity.Layer layer : dluidLayers) {
